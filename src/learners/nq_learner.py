...
class NQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        if self.args.bc:
            ...
            bc_mask = batch["filled"].float()
            bc_terminated = batch["terminated"][:, :].float()
            bc_mask[:, 1:] = bc_mask[:, 1:] * (1 - bc_terminated[:, :-1])
            bc_loss = (F.cross_entropy(mac_out.reshape(-1, a), batch["actions"].squeeze(-1).reshape(-1), reduction="sum") / bc_mask.sum()) / n
